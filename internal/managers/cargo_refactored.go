...
// CargoManagerV2 manages Rust packages via cargo using BaseManager for common functionality.
type CargoManagerV2 struct {
	*BaseManager
}

// NewCargoManagerV2 creates a new cargo manager with the default executor.
func NewCargoManagerV2() *CargoManagerV2 {
	...
	return &CargoManagerV2{
		BaseManager: base,
	}
}

// NewCargoManagerV2WithExecutor creates a new cargo manager with a custom executor for testing.
func NewCargoManagerV2WithExecutor(exec executor.CommandExecutor) *CargoManagerV2 {
	config := ManagerConfig{
		BinaryName:  "cargo",
		VersionArgs: []string{"--version"},
		ListArgs: func() []string {
...
func (c *CargoManagerV2) ListInstalled(ctx context.Context) ([]string, error) {
...
func (c *CargoManagerV2) parseListOutput(output []byte) []string {
...
func (c *CargoManagerV2) Install(ctx context.Context, name string) error {
	return c.ExecuteInstall(ctx, name)
}

// Uninstall removes a cargo package.
func (c *CargoManagerV2) Uninstall(ctx context.Context, name string) error {
	return c.ExecuteUninstall(ctx, name)
}

// IsInstalled checks if a specific package is installed.
func (c *CargoManagerV2) IsInstalled(ctx context.Context, name string) (bool, error) {
...
func (c *CargoManagerV2) Search(ctx context.Context, query string) ([]string, error) {
...
func (c *CargoManagerV2) parseSearchOutput(output []byte) []string {
	var packages []string
...
func (c *CargoManagerV2) Info(ctx context.Context, name string) (*PackageInfo, error) {
...
func (c *CargoManagerV2) parseInfoOutput(output []byte, name string) *PackageInfo {
	scanner := bufio.NewScanner(strings.NewReader(string(output)))
	if !scanner.Scan() {
		return nil
	}
...
func (c *CargoManagerV2) GetInstalledVersion(ctx context.Context, name string) (string, error) {
...
func (c *CargoManagerV2) extractVersion(output []byte, name string) string {
	lines := strings.Split(string(output), "\n")
	for _, line := range lines {
		line = strings.TrimSpace(line)
