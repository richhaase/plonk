...
// NpmManagerV2 manages NPM packages using BaseManager for common functionality.
type NpmManagerV2 struct {
	*BaseManager
}

// NewNpmManagerV2 creates a new NPM manager with the default executor.
func NewNpmManagerV2() *NpmManagerV2 {
...
// NewNpmManagerV2WithExecutor creates a new NPM manager with a custom executor for testing.
func NewNpmManagerV2WithExecutor(exec executor.CommandExecutor) *NpmManagerV2 {
	config := ManagerConfig{
		BinaryName:  "npm",
		VersionArgs: []string{"--version"},
		ListArgs: func() []string {
...
func (n *NpmManagerV2) ListInstalled(ctx context.Context) ([]string, error) {
...
func (n *NpmManagerV2) parseListOutput(output []byte) []string {
...
func (n *NpmManagerV2) Install(ctx context.Context, name string) error {
	return n.ExecuteInstall(ctx, name)
}

// Uninstall removes a global NPM package.
func (n *NpmManagerV2) Uninstall(ctx context.Context, name string) error {
	return n.ExecuteUninstall(ctx, name)
}

// IsInstalled checks if a specific package is installed globally.
func (n *NpmManagerV2) IsInstalled(ctx context.Context, name string) (bool, error) {
...
func (n *NpmManagerV2) Search(ctx context.Context, query string) ([]string, error) {
...
func (n *NpmManagerV2) parseSearchOutput(output []byte) []string {
	result := strings.TrimSpace(string(output))
	if result == "" || result == "[]" {
		return []string{}
	}
...
func (n *NpmManagerV2) Info(ctx context.Context, name string) (*PackageInfo, error) {
...
func (n *NpmManagerV2) parseInfoOutput(output []byte, name string) *PackageInfo {
...
func (n *NpmManagerV2) extractDependencies(jsonOutput string) []string {
...
func (n *NpmManagerV2) GetInstalledVersion(ctx context.Context, name string) (string, error) {
...
func (n *NpmManagerV2) getVersionFromLS(ctx context.Context, name string) (string, error) {
