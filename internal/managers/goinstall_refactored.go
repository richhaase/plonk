...
// GoInstallManagerV2 manages Go packages using BaseManager for common functionality.
type GoInstallManagerV2 struct {
	*BaseManager
}

// NewGoInstallManagerV2 creates a new go install manager with the default executor.
func NewGoInstallManagerV2() *GoInstallManagerV2 {
	...
	return &GoInstallManagerV2{
		BaseManager: base,
	}
}

// NewGoInstallManagerV2WithExecutor creates a new go install manager with a custom executor for testing.
func NewGoInstallManagerV2WithExecutor(exec executor.CommandExecutor) *GoInstallManagerV2 {
	config := ManagerConfig{
		BinaryName:  "go",
		VersionArgs: []string{"version"},
		ListArgs: func() []string {
...
func (g *GoInstallManagerV2) IsAvailable(ctx context.Context) (bool, error) {
...
func (g *GoInstallManagerV2) getGoBinDir(ctx context.Context) (string, error) {
	// First try GOBIN
...
func (g *GoInstallManagerV2) ListInstalled(ctx context.Context) ([]string, error) {
...
func (g *GoInstallManagerV2) isGoBinary(ctx context.Context, binaryPath string) bool {
...
func (g *GoInstallManagerV2) Install(ctx context.Context, name string) error {
...
func (g *GoInstallManagerV2) Uninstall(ctx context.Context, name string) error {
	binDir, err := g.getGoBinDir(ctx)
	if err != nil {
		return err
	}
...
func (g *GoInstallManagerV2) IsInstalled(ctx context.Context, name string) (bool, error) {
	binDir, err := g.getGoBinDir(ctx)
	if err != nil {
		return false, err
	}
...
func (g *GoInstallManagerV2) Search(ctx context.Context, query string) ([]string, error) {
...
func (g *GoInstallManagerV2) Info(ctx context.Context, name string) (*PackageInfo, error) {
	binDir, err := g.getGoBinDir(ctx)
	if err != nil {
		return nil, err
	}
...
func (g *GoInstallManagerV2) GetInstalledVersion(ctx context.Context, name string) (string, error) {
	binDir, err := g.getGoBinDir(ctx)
	if err != nil {
		return "", err
	}
