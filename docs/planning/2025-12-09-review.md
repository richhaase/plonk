...
func getMetadataString(...)  // Line 189 - unexported duplicate
```
...
A comprehensive deadcode analysis identified **45+ unreachable functions** in production code. These fall into several categories:
...
| `internal/commands/status.go:209-231`       | `sortItems`, `sortItemsByManager`                                                                         | Duplicates of functions in `output/status_formatter.go` |
...
| `internal/output/utils.go`                  | `TruncateString`, `FormatValidationError`, `FormatNotFoundError`                                          | Utility and error formatters never called               |
...
| `internal/resources/dotfiles/manager.go`    | `Manager.computeFileHash`, `Manager.createCompareFunc`                                                    | Internal helpers never called                           |
...
**Impact:** ~45 functions, ~500+ lines of dead code
...
3. **Remove extensive dead code (Category A)** - ~35 unreachable functions across output/, commands/, resources/
