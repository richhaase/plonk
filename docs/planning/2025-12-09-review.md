...
### ~~2. IsInstalled Is O(n) Per Check~~ ✅ RESOLVED

**Status:** Fixed as of 2025-12-11

**Original Issue:** `IsInstalled()` called `ListInstalled()` every time, which would be O(n) subprocess calls.

**Resolution:** Upon investigation, `IsInstalled()` was **never called** in production code — it was dead code. The actual reconciliation flow uses `ListInstalled()` once per manager, not once per package. Removed `IsInstalled()` from the interface and `GenericManager` implementation.
...
### ~~1. Type Conversion Ceremony~~ Won't Fix

**File:** `internal/commands/doctor.go:83-96`

**Original Issue:** `diagnostics.HealthCheck` and `output.HealthCheck` are structurally identical, requiring field-by-field conversion.

**Decision:** Won't fix. The conversion is ~15 lines in one place. Eliminating it would require coupling `output` → `diagnostics` or creating a shared types package. The tradeoff isn't worth it for this isolated case.

---

### ~~2. Registry Created Multiple Times~~ ✅ RESOLVED

**Status:** Fixed as of 2025-12-11

**Original Issue:** `packages.NewManagerRegistry()` was called in many places, and the function name suggested creating new instances when it actually returned a singleton.

**Resolution:** Renamed `NewManagerRegistry()` to `GetRegistry()` to accurately reflect singleton semantics. All call sites updated.
...
| ~~`generic.go:74-82`~~ | ~~Duplicate idempotent check logic—could extract to helper~~                          | Won't Fix - explicit code documents intent better than collapsed helper | ~~Low~~      |
...
2. ~~**Cache ListInstalled results**~~ ✅ Removed unused `IsInstalled()` — actual code already efficient
3. ~~**Reduce type conversion boilerplate**~~ - Won't fix; ~15 lines in one place, coupling tradeoff not worth it

### ~~Low Priority~~ ✅ ALL COMPLETED

1. ~~**Registry singleton/DI**~~ ✅ Renamed to `GetRegistry()` for clarity
...
6. ~~Extract idempotent check helper~~ - Explicit code documents intent better
7. ~~Type conversion boilerplate~~ - ~15 lines in one place, coupling tradeoff not worth it
