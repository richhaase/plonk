...
| Flag | Short | Type | Default | Description |
|------|-------|------|---------|-------------|
| `--packages` | | bool | `false` | Show only package status |
| `--dotfiles` | | bool | `false` | Show only dotfile status |
| `--unmanaged` | | bool | `false` | Show only unmanaged items |
...
- `--unmanaged` and `--missing`: **Mutually exclusive** - cannot use both
...
| Flag | Short | Type | Default | Description |
|------|-------|------|---------|-------------|
| `--unmanaged` | | bool | `false` | Show only unmanaged packages |
| `--missing` | | bool | `false` | Show only missing packages |

### Flag Behavior
- `--unmanaged` and `--missing`: **Mutually exclusive**
...
| Flag | Short | Type | Default | Description |
|------|-------|------|---------|-------------|
| `--unmanaged` | | bool | `false` | Show only unmanaged dotfiles |
...
| `--packages` | status, apply | Consistent |
| `--dotfiles` | status, apply | Consistent |
| `--unmanaged` | status, packages, dotfiles | Consistent |
...
|---------|-------------------------|-------------|
| `status` | `--unmanaged`, `--missing` | Runtime validation |
| `packages` | `--unmanaged`, `--missing` | Runtime validation |
| `dotfiles` | `--unmanaged`, `--missing` | Runtime validation |
| `apply` | `--packages`, `--dotfiles` | Cobra's `MarkFlagsMutuallyExclusive` |

**Inconsistency:** `apply` uses Cobra's built-in mutual exclusivity, while status commands use runtime validation.
...
| `status` | - | `--packages`, `--dotfiles`, `--unmanaged`, `--missing` | `st` | - |
| `packages` | - | `--unmanaged`, `--missing` | `p` | - |
| `dotfiles` | - | `--unmanaged`, `--missing` | `d` | - |
| `diff` | `[file]` | - | - | - |
...
2. **Standardize mutual exclusivity enforcement** - use Cobra's `MarkFlagsMutuallyExclusive` consistently
3. **Add `--dry-run` to `clone`** to preview what would be cloned/installed
4. **Consider adding aliases** to commonly used commands (e.g., `i` for install, `u` for upgrade)
