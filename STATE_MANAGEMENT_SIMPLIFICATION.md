...
Before making any changes, we need a clear understanding of the current state management architecture.

### Phase 0.1: Map Core Components

1.  **Action:** Identify and list the primary files and types involved in state management:
    *   `internal/state/reconciler.go`
    *   `internal/state/types.go` (especially `Result`, `Item`, `Status`)
    *   `internal/state/dotfile_provider.go`
    *   `internal/state/package_provider.go`
    *   `internal/interfaces/core.go` (specifically the `Provider` interface)
    *   `internal/interfaces/package_manager.go` (related to `PackageConfigLoader`)
    *   `internal/mocks/` (any mocks related to state/providers)
2.  **Action:** For each identified component, briefly describe its role in the state reconciliation process.

### Phase 0.2: Trace Reconciliation Flow

1.  **Action:** Trace the execution flow for a typical `plonk status` command:
    *   How is the `Reconciler` initialized?
    *   How are `Provider` implementations registered?
    *   How does `ReconcileProvider` or `ReconcileAll` work?
    *   How are `Result` and `Item` types populated and used?
    *   Where does the "diffing logic" (configured vs. actual) reside?
2.  **Action:** Document the layers of indirection and any complex logic identified.

### Phase 0.3: Document Findings

1.  **Action:** Create a new document (e.g., `STATE_ANALYSIS.md`) to list all findings from Phase 0.1 and 0.2. This will serve as our working document for this refactor.
...
### Phase 1: Simplify Providers

This phase targets the `Provider` interface and its implementations.

1.  **Target: `Provider` Interface (`internal/interfaces/core.go`)**
    *   **Action:** Analyze the `Provider` interface. Does it truly require polymorphism, or can its functionality be directly integrated into the `Reconciler` or a simpler helper?
...
3.  **Verification:** Run `just test` and `just test-ux`. Manually test `plonk status` and `plonk sync`.
