...
func ValidateYAML(content []byte) error {
	...
	if trimmed == "" || isOnlyComments(trimmed) {
		return nil 		return nil // Empty or comment-only YAML is valid.
	}
	...
	decoder.KnownFields(false) 	decoder.KnownFields(false) // Allow unknown fields for flexibility.
...
func validateHomebrewPackages(homebrew interface{}) error {
	...
	if !ok {
		return nil 		return nil // Skip if not a map.
	}
...
func validateDotfiles(dotfiles interface{}) error {
	...
	if !ok {
		return nil 		return nil // Skip if not a list.
	}
